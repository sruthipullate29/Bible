...
function drawReference(
  ...
  ctx.save()
  ctx.font = `${ref.fontWeight} ${ref.fontSize}px "${ref.fontFamily}", "Noto Sans Telugu", "Nirmala UI", "Gautami", "Telugu Sangam MN", sans-serif`
...
function drawVerseText(
  ...
  ctx.save()
  ctx.font = `${vt.fontWeight} ${vt.fontSize}px "${vt.fontFamily}", "Noto Sans Telugu", "Nirmala UI", "Gautami", "Telugu Sangam MN", serif`
...
function measureVerseHeight(
  ...
  ctx.restore()
  return {
    height: Math.max(lineHeightPx, totalHeight),
...
export function computeVerseLayoutMetrics(
  ...
  // Auto-fit loop: automatically scale down fonts so both verses fit slide bounds perfectly
  let currentScaledTheme = scaledTheme
  let verseMetrics = measureVerseHeight(ctx, currentScaledTheme, verse, textRectW)
  let referenceHeight = currentScaledTheme.reference.fontSize * 1.5
  let blockHeight = currentScaledTheme.reference.position === "above"
    ? referenceHeight + verseMetrics.height
    : currentScaledTheme.reference.position === "below"
      ? verseMetrics.height + referenceGap + referenceHeight
      : verseMetrics.height + referenceHeight

  let iterations = 0
  while (blockHeight > textRectH && iterations < 8 && currentScaledTheme.verseText.fontSize > 11 * scale) {
    const ratio = Math.max(0.65, Math.min(0.94, (textRectH * 0.96) / blockHeight))
    currentScaledTheme = {
      ...currentScaledTheme,
      verseText: {
        ...currentScaledTheme.verseText,
        fontSize: Math.max(11 * scale, currentScaledTheme.verseText.fontSize * ratio),
      },
      reference: {
        ...currentScaledTheme.reference,
        fontSize: Math.max(11 * scale, currentScaledTheme.reference.fontSize * Math.max(0.8, ratio)),
      },
    }
    verseMetrics = measureVerseHeight(ctx, currentScaledTheme, verse, textRectW)
    referenceHeight = currentScaledTheme.reference.fontSize * 1.5
    blockHeight = currentScaledTheme.reference.position === "above"
      ? referenceHeight + verseMetrics.height
      : currentScaledTheme.reference.position === "below"
        ? verseMetrics.height + referenceGap + referenceHeight
        : verseMetrics.height + referenceHeight
  ...
  scaledTheme = currentScaledTheme
  ...
  ctx.save()
  ctx.font = `${scaledTheme.reference.fontWeight} ${scaledTheme.reference.fontSize}px "${scaledTheme.reference.fontFamily}", "Noto Sans Telugu", "Nirmala UI", "Gautami", "Telugu Sangam MN", sans-serif`
